 ...
 *      using hash(advertiserId) as seed. Same advertiser → same spec
 *      every time (idempotent re-renders, no surprise visual changes).
 *
 * No AI is called from this module. The photo enhancement + analysis
 * happens upstream (Gemini 2.5 Flash Image edge function); this module
 * just consumes the resulting vibes.
 *
 * NOTHING in this file is imported by the legacy AdvertiserAd.tsx render
 * pipeline. It runs in parallel and is only wired up once we've
 * validated A/B output in Phase 6.
 */
...
import { FAMILIES, DEFAULT_FAMILY } from "./families";
